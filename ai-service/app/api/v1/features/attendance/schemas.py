...
class AttendanceRecognitionRequest(BaseModel):
    """
    Payload ai-service sends to api-service after a stable face recognition.

    api-service owns the attendance business rules, so this request only
    describes what ai-service recognized from the camera/Qdrant pipeline.
    """
    ...
    image_url: str | None = Field(
        default=None,
        description="Optional evidence image URL if ai-service stores snapshots",
    )
    raw_result: dict[str, Any] | None = Field(
        default=None,
        description="Extra debug metadata from ai-service",
    )
